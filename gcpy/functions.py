...
def query_to_gcs(
    ...
    bq_dataset = client.create_dataset(
        bq_dataset, timeout=30, exists_ok=True
    )  # Make an API request.
    ...
    helpers.bq_to_gcs(
        project, dataset, tablename, gcs_path, location=location, client=client
    )
...
def gcs_to_bq(
    ...
    load_job = client.load_table_from_uri(
        source_gcs_path, dataset.table(target_bq_tablename), job_config=job_config
    )  # API request
...
def sharded_gcs_csv_to_pd(source_gcs_path: str, file_prefix: str):
    ...
    prefix = "/".join(source_gcs_path.split("/")[3:-1]) + "/" + file_prefix
    ...
    file_list = [
        "gs://" + bucketName + "/" + file.name for file in files if "." in file.name
    ]
